...
public class JokerKeyBoradHelper {
    ...
    public JokerKeyBoradHelper(Context context, JokerCustomKeyBoradView keyboardView) {
        this(context, keyboardView, null);
    }

    public JokerKeyBoradHelper(Context context, JokerCustomKeyBoradView keyboardView, KeyboardCallBack callBack) {
        this.context = context;
        k1 = new Keyboard(context, R.xml.joker_keyboard_temp);//据Keyboard的xml布局绑定
    ...
    private boolean defaultLogic(char primaryCode, Editable editable, int start, int end, boolean isequals) {
        ...
        boolean b1 = "0".equals(s) || "1".equals(s) || "2".equals(s) || "3".equals(s) || "4".equals(s)
                || "5".equals(s) || "6".equals(s) || "7".equals(s) || "8".equals(s) || "9".equals(s);

        if (content.contains(".")){

            String[] splitAdd = content.split("\\+");
            if (splitAdd.length == 2) {
                String[] splitTwoAdd = splitAdd[1].split("\\.");
                if (splitTwoAdd.length==2){
                    if (splitTwoAdd[1].length() == 2) {
                        if (b1) {
                            Log.i("JokerKeyBoradHelper", "输入的内容（（（（" + s);
                            return true;
                        }
                    }
                }
            ...
            String[] splitFu = content.split("-");
            if (splitFu.length == 2) {
                String[] splitTwoFu = splitFu[1].split("\\.");
                if (splitTwoFu.length==2){
                    if (splitTwoFu[1].length() == 2) {
                        if (b1) {
                            Log.i("JokerKeyBoradHelper", "输入的内容（" + s);
                            return true;
                        }
        ...
        if (content.matches("\\d+")) {
            if (content.length() >= 8) {
                if (b1) {
                    Log.i("JokerKeyBoradHelper", "输入的内容不能太多" + s);
                    return true;
                }
                // editable.insert(start, Character.toString(primaryCode));
                //  //editable.delete(content.length ()-1, content.length ());
            }
        } else if (content.contains("-") || content.contains("+")) {
            String[] split = content.split("-");
            if (split.length == 2) {
                if (split[1].length() >= 8&&!split[1].contains(".")) {
                    if (b1) {
                        Log.i("JokerKeyBoradHelper", "输入的内容&&" + s);
                        return true;
                    }
            ...
            if (split2.length == 2) {
                if (split2[1].length() >= 8&&!split2[1].contains(".")) {
                    if (b1) {
                        Log.i("JokerKeyBoradHelper", "输入的内容==" + s);
                        return true;
                    }

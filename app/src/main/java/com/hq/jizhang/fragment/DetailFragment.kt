...
class DetailFragment : BaseFragment() {
    ...
    open fun getMonthMoneyData(){
        val month = fg_detail_tv_month.text.substring(0 , 2)
        val findAll = LitePal.findAll(DetailSqlBean::class.java)
        val sumOfIncome = findAll.filter { it.month == month&&it.type==CommonConstant.MainActivity.TYPE_INCOME }.sumOf { it.money.toBigDecimal() }
        val sumOfDisburse = findAll.filter { it.month == month&&it.type==CommonConstant.MainActivity.TYPE_DISBURSE }.sumOf { it.money.toBigDecimal() }
